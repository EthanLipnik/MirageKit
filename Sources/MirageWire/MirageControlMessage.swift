...
package enum MirageControlMessageLimits {
    package static let maxPayloadBytes = 8 * 1024 * 1024
    package static let maxLargeMetadataPayloadBytes = 32 * 1024 * 1024
    package static let maxInlineAssetPayloadBytes = 4 * 1024 * 1024
    package static let maxFrameBytes = maxLargeMetadataPayloadBytes + 5
    package static let maxReceiveBufferBytes = 64 * 1024 * 1024
}
...
package enum ControlMessageType: UInt8, Codable {
    ...
    case desktopCursorPresentationChange = 0xA7 // Client → Host: Update desktop cursor mode
    case qualityTestRequest = 0xA4 // Client → Host: Legacy quality-test request
    case qualityTestResult = 0xA5 // Host → Client: Legacy quality-test result
    case qualityTestStageComplete = 0xA8 // Host → Client: Legacy quality-test stage completion
    case qualityTestCancel = 0xA9 // Client → Host: Legacy quality-test cancel
...
package struct ControlMessage: Codable {
    ...
    private static func maxPayloadBytes(for type: ControlMessageType) -> Int {
        ...
        case .appListProgress:
            MirageControlMessageLimits.maxLargeMetadataPayloadBytes
        case .hostHardwareIcon, .hostWallpaper:
            MirageControlMessageLimits.maxInlineAssetPayloadBytes
        default:
