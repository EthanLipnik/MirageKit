    ...
    struct Writer {
        ...
        mutating func appendInt32(_ value: Int) throws {
            guard let raw = Int32(exactly: value) else {
                throw MirageCore.MirageError.protocolError("Input payload integer overflow for Int32 field")
            }
        ...
        mutating func appendString(_ value: String) throws {
            ...
            guard let length = UInt16(exactly: utf8.count) else {
                throw MirageCore.MirageError.protocolError("Input string field length overflow")
            }
            appendUInt16(length)
            data.append(utf8)
        }

        mutating func appendKeyEvent(_ event: MirageInput.MirageKeyEvent) throws {
        ...
        mutating func appendHostSystemActionRequest(_ request: MirageInput.MirageHostSystemActionRequest) throws {
        ...
        mutating func appendMouseEvent(_ event: MirageInput.MirageMouseEvent) {
        ...
        mutating func appendPointerSampleBatch(_ batch: MirageInput.MiragePointerSampleBatch) throws {
            ...
            guard let sampleCount = UInt16(exactly: batch.samples.count) else {
                throw MirageCore.MirageError.protocolError("Pointer sample batch exceeds \(UInt16.max) samples")
            }
        ...
        mutating func appendPointerSample(_ sample: MirageInput.MiragePointerSample) {
        ...
        mutating func appendStylusEvent(_ event: MirageInput.MirageStylusEvent?) {
        ...
        mutating func appendScrollEvent(_ event: MirageInput.MirageScrollEvent) {
        ...
        mutating func appendMagnifyEvent(_ event: MirageInput.MirageMagnifyEvent) {
        ...
        mutating func appendRotateEvent(_ event: MirageInput.MirageRotateEvent) {
        ...
        mutating func appendSwipeEvent(_ event: MirageInput.MirageSwipeEvent) {
            appendDouble(Double(event.deltaX))
            appendDouble(Double(event.deltaY))
            appendBool(event.location != nil)
            if let location = event.location {
        ...
        mutating func appendResizeEvent(_ event: MirageInput.MirageResizeEvent) {
        ...
        mutating func appendRelativeResizeEvent(_ event: MirageInput.MirageRelativeResizeEvent) throws {
            appendUInt32(event.windowID)
        ...
        mutating func appendPixelResizeEvent(_ event: MirageInput.MiragePixelResizeEvent) throws {
            appendUInt32(event.windowID)
            try appendInt32(event.pixelWidth)
            try appendInt32(event.pixelHeight)
            appendDouble(event.timestamp)
