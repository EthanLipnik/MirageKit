//
//  MirageWindowStreamingMessages.swift
//  MirageWire
//
//  Created by Ethan Lipnik on 6/5/26.
//
//  Window streaming control message definitions.
//
...
package struct StartStreamMessage: Codable {
    ...
    package var audioConfiguration: MirageMedia.MirageAudioConfiguration?
...
package struct WindowListMessage: Codable {
    ...
    package let windows: [MirageMedia.MirageWindow]

    /// Creates a complete window inventory payload.
    package init(windows: [MirageMedia.MirageWindow]) {
...
package struct WindowUpdateMessage: Codable {
    ...
    package let added: [MirageMedia.MirageWindow]

    /// Window IDs that are no longer available.
    package let removed: [WindowID]

    /// Existing windows with refreshed metadata.
    package let updated: [MirageMedia.MirageWindow]

    /// Creates an incremental window inventory update payload.
    package init(added: [MirageMedia.MirageWindow], removed: [WindowID], updated: [MirageMedia.MirageWindow]) {
        self.added = added
