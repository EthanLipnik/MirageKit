//
//  MiragePriorityInputMessage.swift
//  MirageWire
//
...
package struct MiragePriorityInputEnvelope: Equatable, Sendable {
    ...
    package func serialize() throws -> Data {
        guard inputPayload.count <= MirageControlMessageLimits.maxPayloadBytes else {
            throw MirageCore.MirageError.protocolError(
                "Priority input payload exceeds \(MirageControlMessageLimits.maxPayloadBytes) bytes."
            )
        }
    ...
    package static func deserialize(_ data: Data) throws -> MiragePriorityInputEnvelope {
        var reader = PriorityInputEnvelopeReader(data: data)
        let version = try reader.readUInt8()
        guard version == Self.version else {
            throw MirageCore.MirageError.protocolError("Unsupported priority input envelope version \(version).")
        }
        guard let kind = MiragePriorityInputEnvelopeKind(rawValue: try reader.readUInt8()) else {
            throw MirageCore.MirageError.protocolError("Invalid priority input envelope kind.")
        }
        guard let deliveryClass = MiragePriorityInputDeliveryClass(rawValue: try reader.readUInt8()) else {
            throw MirageCore.MirageError.protocolError("Invalid priority input delivery class.")
        }
        ...
        guard reader.isAtEnd else {
            throw MirageCore.MirageError.protocolError("Priority input envelope has trailing bytes.")
        }
    ...
    package func inputControlMessage() throws -> ControlMessage {
        guard kind == .input else {
            throw MirageCore.MirageError.protocolError("Priority input envelope does not carry input.")
        }
...
private struct PriorityInputEnvelopeReader {
    ...
    mutating func readUInt8() throws -> UInt8 {
        guard offset < data.count else { throw MirageCore.MirageError.protocolError("Truncated priority input envelope.") }
        defer { offset += 1 }
    ...
    mutating func readData(count: Int) throws -> Data {
        guard count >= 0, offset + count <= data.count else {
            throw MirageCore.MirageError.protocolError("Truncated priority input envelope payload.")
        }
    ...
    private mutating func readInteger<T: FixedWidthInteger>(_ type: T.Type) throws -> T {
        ...
        guard offset + byteCount <= data.count else {
            throw MirageCore.MirageError.protocolError("Truncated priority input envelope integer.")
        }
        let start = data.index(data.startIndex, offsetBy: offset)
