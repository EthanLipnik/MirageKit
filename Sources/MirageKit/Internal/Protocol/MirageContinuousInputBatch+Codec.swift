...
package extension MirageContinuousInputBatch {
    ...
    func serialize() throws -> Data {
        guard samples.count <= UInt16.max else {
            throw MirageError.protocolError("Continuous input batch exceeds \(UInt16.max) samples")
        }
    ...
    static func deserialize(_ data: Data) throws -> MirageContinuousInputBatch {
        ...
        guard version == Self.wireVersion else {
            throw MirageError.protocolError("Unsupported continuous input batch version \(version)")
        }
        ...
        guard let kind = Kind(rawValue: try reader.readUInt8()) else {
            throw MirageError.protocolError("Invalid continuous input kind")
        }
        ...
        guard pointerPhaseRaw == UInt8.max || pointerPhase != nil else {
            throw MirageError.protocolError("Invalid continuous pointer phase")
        }
...
private struct ContinuousInputReader {
    ...
    mutating func readUInt8() throws -> UInt8 {
        guard offset < data.count else {
            throw MirageError.protocolError("Continuous input payload ended unexpectedly")
        }
        defer { offset += 1 }
        return data[offset]
    }
    ...
    mutating func requireFinished() throws {
        guard offset == data.count else {
            throw MirageError.protocolError("Continuous input payload has trailing bytes")
        }
    }
