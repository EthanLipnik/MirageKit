//
//  MirageLogArchiverDiagnosticsTests.swift
//  MirageDiagnostics
//
...
@Suite("Log Archiver Diagnostics")
struct MirageLogArchiverDiagnosticsTests {
    @Test("Truncation keeps log head tail and marker")
    func truncationKeepsLogHeadTailAndMarker() throws {
        let lines = (1 ... 8).map { "line-\($0)" }
        let data = MirageDiagnostics.MirageLogArchiver.truncatedLogData(
            lines: lines,
            keeping: 4,
    ...
    func zipArchiveContainsExpectedRecords() throws {
        let archive = try MirageDiagnostics.MirageLogArchiver.zipArchiveData(
            entries: [
    ...
    func supportArchiveIncludesAdditionalEntries() throws {
        let archiveURL = try MirageDiagnostics.MirageLogArchiver.exportArchive(
            ...
            additionalEntries: [
                MirageDiagnostics.MirageLogArchiveEntry(name: "MetricKitDiagnostics.txt", text: "metric\n"),
                MirageDiagnostics.MirageLogArchiveEntry(name: "PreviousSession.log", text: "previous\n"),
            ]
        )
