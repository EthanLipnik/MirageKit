...
struct MirageBitrateQualityMapperTests {
    ...
    func highRefreshUsesSixtyHertzQualityReference() {
        let sixty = MirageBitrateQualityMapper.derivedQualities(
            targetBitrateBps: 96_000_000,
            width: 2752,
            height: 2064,
            frameRate: 60
        )
        let oneTwenty = MirageBitrateQualityMapper.derivedQualities(
            targetBitrateBps: 96_000_000,
            width: 2752,
            height: 2064,
            frameRate: 120
    ...
    func highRefreshNoLongerAppliesQualityScale() {
        #expect(MirageBitrateQualityMapper.frameRateScale(frameRate: 120, bpp: 0.05) == 1.0)
        #expect(MirageBitrateQualityMapper.frameRateScale(frameRate: 90, bpp: 0.05) == 1.0)
        #expect(MirageBitrateQualityMapper.frameRateScale(frameRate: 60, bpp: 0.05) == 1.0)
    }
}
