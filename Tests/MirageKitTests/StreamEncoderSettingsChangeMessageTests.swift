...
struct StreamEncoderSettingsChangeMessageTests {
    ...
    func olderEncoderSettingsMessagesDecodeWithoutBitrateCeiling() throws {
        ...
        let message = try JSONDecoder().decode(MirageWire.StreamEncoderSettingsChangeMessage.self, from: data)
    ...
    func encoderSettingsMessagesEncodeBitrateCeilingWhenPresent() throws {
        let message = MirageWire.StreamEncoderSettingsChangeMessage(
            streamID: 9,
            bitrate: 64_000_000,
        ...
        let decoded = try JSONDecoder().decode(MirageWire.StreamEncoderSettingsChangeMessage.self, from: data)
