...
- Desktop stream startup uses explicit `desktopStreamStarted`, `desktopStreamFailed`, and `desktopStreamStopped` control messages so startup rejection is distinguishable from later transport loss. Each full desktop session carries a host-minted `desktopSessionID`; fresh desktop starts replace that UUID, while resize updates stay inside the same session UUID.
