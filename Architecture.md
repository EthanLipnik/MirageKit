...
- Desktop stream startup uses explicit `desktopStreamStarted`, `desktopStreamFailed`, and `desktopStreamStopped` control messages so startup rejection is distinguishable from later transport loss. Each full desktop session carries a host-minted `desktopSessionID`; fresh desktop starts replace that UUID, while resize updates stay inside the same session UUID. Legacy peers that predate that field are treated as a synthesized per-stream legacy session scope so mixed-version desktop start/stop still complete, while fresh peers keep full session replacement filtering.
...
The host keeps one shared app-stream virtual display mirrored from the current physical-display set. App-stream capture is display-based, not `desktopIndependentWindow`-based: each visible slot resolves a window cluster made of the selected primary window plus attached supplementary descendants, captures the mirrored display through a display filter limited to that cluster, and crops the presentation region with `sourceRect` / `destinationRect`. The encoded canvas stays fixed for the life of the slot stream; live app resize and supplementary-window churn update only the filtered display-capture geometry, so the client receives a stable encoded stream with a moving `contentRect` instead of a resize-driven encoder reset and recovery keyframe. App-stream resize requests are dispatched immediately, and the host preserves the slot's chosen preset aspect ratio while shrinking the window to the largest accepted fit inside the mirrored display's visible bounds. The wire/session model keeps the visible slot's `streamID` stable even when the bound host window changes, and close/remove propagation keys off that stream identity while still carrying the current `windowID` as window metadata.

The host also exposes a host-local capture benchmark path for diagnostics. That benchmark is not a client session: it acquires an exact-size shared virtual display through `SharedVirtualDisplayManager`'s benchmark consumer, places a dedicated high-motion source window on that display, captures the display through `ScreenCaptureKit`, and feeds frames into a local `VideoEncoder` session without any network transport. Each benchmark stage records separate capture FPS, encode FPS, effective FPS, and capture-drop / copy telemetry so the host app can distinguish display-capture bottlenecks from encoder bottlenecks at each tested resolution.
...
Foreground activation recovery on iOS and visionOS is also desktop-session scoped. Pending activation recovery is tagged with the desktop session that was active when the app backgrounded, and recovery is only requested after foregrounding if that same desktop session is still active and has already presented at least one frame.
...
- explicit client-requested desktop stops suppress terminal startup-failure surfacing so a user-initiated exit cannot later present a bounded-recovery failure for the same stream
...
- desktop stop requests and `desktopStreamStopped` confirmations are validated against the active `desktopSessionID`, so a late stop from a superseded desktop session cannot tear down the replacement session even if the host reuses the same `streamID`
...
- post-resize recovery keeps keyframe-only decode admission armed only until the first newly presented frame, but the post-resize recovery state stays active until the decoder reports a short successful post-keyframe streak; once the short post-resize grace window expires, any new decode-error threshold triggers immediate soft recovery, and coalesced follow-up desktop resizes stay queued until that first presented frame lands
- app-owned adaptive bitrate recovery is shared by automatic quality mode and custom recovery mode; the receiver-health controller reads client metrics snapshots, ignores startup and resize/scale reconfiguration windows while the client startup-critical section is active, applies the 15%/25% backoff rules only for transport-pressure signals such as send-queue growth, pacing delay, packet drops, or delivery falling behind the host's actual encoded cadence, and probes back upward only when transport is healthy, decode is healthy, rendered cadence is at least `95%` of target, and the bottleneck remains `unknown` or `networkBound`
...
- automatic-mode startup begins from a path-seeded bitrate (`64 Mbps` on AWDL, wired, and loopback paths; `32 Mbps` on Wi-Fi; `14 Mbps` on cellular, other, and unknown paths); when the user-selected preset is `.auto` and the control path is cellular, other, or unknown, startup keeps the geometry-derived ceiling but also applies a transient stream-scale multiplier derived from the seeded bitrate versus that ceiling so first paint is conservative without making the lower geometry sticky for the rest of the session
